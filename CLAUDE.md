...
- **State ownership (non-streaming, greeting/fallback):** `pipeline.process_turn` owns `PROCESSING` only — it runs STT+LLM+synth and returns audio with the session still in `PROCESSING`. `AriClient._play_audio` drives `PROCESSING → SPEAKING → LISTENING` (and `ANSWER → SPEAKING` for the greeting) around actual RTP playback.
