...
async def test_get_events_returns_formatted_string(calendar):
    respx.get(f"https://graph.microsoft.com/v1.0/users/{calendar._user_email}/calendarView").mock(
...
async def test_create_event_returns_confirmation(calendar):
    respx.post(f"https://graph.microsoft.com/v1.0/users/{calendar._user_email}/events").mock(
        return_value=httpx.Response(201, json={"id": "evt1", "subject": "Arzt"})
    )
